...
public class OmeroConnect {
    ...
    private mcib3d.utils.Logger.AbstractLog logger = new IJLog();
...
/*
    ...
    public void connect() throws Exception {
        ...
        ExperimenterData user = gateway.connect(cred);
        // TODO set a specific group
        securityContext = new SecurityContext(user.getGroupId());
        if (!gateway.isConnected()) IJ.log("Pb Connection to server " + creds[1]);
    }

    private String[] readCredentials(String file) {
        String[] creds = new String[4];
        try {
            BufferedReader reader = new BufferedReader(new FileReader(new File(file)));
            creds[0] = reader.readLine();
            creds[1] = reader.readLine();
            creds[2] = reader.readLine();
            creds[3] = reader.readLine();
            reader.close();
    ...
    private String[] getCredentials() {
        ...
        String port = ij.Prefs.get("OMERO.TB.port.int", "4064");

        return new String[]{user, server, port, pass};
    }
    ...
    public long getUserId(String name) throws ExecutionException, DSAccessException, DSOutOfServiceException {
        // FIND ID OF USER
    ...
    public ImageHandler getImageZ(ImageData image, int t, int c, int zmin, int zmax) throws Exception {
     ...
     * @throws Exception
     */
    public ImageHandler getImageXYZ(ImageData image, int t, int c, int binXY, int binZ, int XMin, int XMax, int YMin, int YMax, int ZMin, int ZMax) throws Exception {
        RawDataFacility rdf = gateway.getFacility(RawDataFacility.class);
        PixelsData pixels = image.getDefaultPixels();
        int sizeZ = pixels.getSizeZ(); // The number of z-sections.
        int sizeT = pixels.getSizeT(); // The number of timepoints.
    ...
    public DatasetData findDataset(String name, ProjectData project, boolean strict) {
    ...
    public ArrayList<DatasetData> findDatasets(ProjectData project) {
        Set<DatasetData> data = project.getDatasets();
    ...
    private ImageData findOneImage(DatasetData data, String name, boolean strict) throws Exception {
        BrowseFacility browse = gateway.getFacility(BrowseFacility.class);
    ...
    public ArrayList<ImageData> findAllImages(DatasetData data) throws Exception {
    ...
    public ArrayList<ImageData> findImagesContainsName(DatasetData data, String name, boolean strict) throws
            Exception {
        ArrayList<ImageData> imageList = new ArrayList<>();
        BrowseFacility browse = gateway.getFacility(BrowseFacility.class);
        Collection<ImageData> images = browse.getImagesForDatasets(securityContext, Arrays.asList(data.getId()));
    ...
    public ProjectData findProject(String name, boolean strict) throws Exception {
        BrowseFacility browse = gateway.getFacility(BrowseFacility.class);
        Collection<ProjectData> projects = browse.getProjects(securityContext);
        for (ProjectData projectData : projects) {
    ...
    public ArrayList<ProjectData> findAllProjects() throws Exception {
    ...
    public void deleteImage(ImageData imageData) throws Exception {
        DataManagerFacility dm = gateway.getFacility(DataManagerFacility.class);
        Response rsp = dm.delete(securityContext, imageData.asIObject()).loop(10, 500);
    }

    public boolean addImageToDataset(String projectName, String datasetName, String dir, String fileName) throws
            Exception {
    ...
    public boolean addImageToDataset(String projectName, String datasetName, String dir, String fileName,
        ...
        config.hostname.set(creds[1]);
        config.port.set(Integer.parseInt(creds[2]));
        config.username.set(creds[0]);
        Encrypt encrypt = new Encrypt();
...
// config.targetId.set(1L);
    ...
    public boolean createDataset(String projectName, String dataName, String description) throws Exception {
        ...
        String[] creds = readCredentials(System.getProperty("user.home") + "/.cache/core/init.cfg");
        ...
        config.hostname.set(creds[0]);
        config.port.set(Integer.parseInt(creds[1]));
        config.username.set(creds[2]);
        config.password.set(creds[3]);
...
// config.targetId.set(1L);
    ...
    public List<FileAnnotationData> getFileAnnotations(ImageData image, ArrayList<String> addUsers) throws DSOutOfServiceException, ExecutionException, DSAccessException {
        //IJ.log("Finding pairs for keyword " + image.getName());
    ...
    public void addFileAnnotation(Dataset dataset, File file) throws java.util.concurrent.ExecutionException, java.io.IOException, DSAccessException, DSOutOfServiceException, ServerError {
    ...
    // TODO check and update
    public void addFileAnnotation(ProjectData project, File file) throws java.util.concurrent.ExecutionException, java.io.IOException, DSAccessException, DSOutOfServiceException, ServerError {
        FileAnnotation fa = createFileAnnotation(file);
    ...
    private FileAnnotation createFileAnnotation(File file) throws java.util.concurrent.ExecutionException, java.io.IOException, DSAccessException, DSOutOfServiceException, ServerError {
        int INC = 262144;
        DataManagerFacility dm = gateway.getFacility(DataManagerFacility.class);
        ...
        fa.setDescription(omero.rtypes.rstring("TEST")); // The description set above e.g. PointsModel
        fa.setNs(omero.rtypes.rstring("test.thomas")); // The name space you have set to identify the file annotation.
